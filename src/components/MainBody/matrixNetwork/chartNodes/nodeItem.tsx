                ...
                {node === 'f1' && status === 'empty' ?
                    ...
                    <Fragment>
                        ...
                        <div className="niw_right">
                            <span className="niwr_name">{node === "user" ? name : nodeData.name}</span>
                            <span className="niwr_lv">Lv. {node === "user" ? level : nodeData.level}</span>
                        </div>
                    </Fragment>
                }
            </div>
...
const NodeItemWrap = memo(styled.div`
    ...
    border-radius:50px;
    ${(props: any) => props.node === 'sponsor' ?
        css`height:88.10%; background-color:${Colors.green1};`
        :
        props.node === 'user' ?
            css`height:56%; background-color:${Colors.green1};`
            :
            props.status === 'disable' ?
                css`height:43.55%; background-color:${Colors.red}; `
                :
                props.status === 'active' ?
                    css`height:43.55%; background-color:${Colors.blue};`
                    :
                    css`height:43.55%; background-color:none;`
    };
    width:25%;
    position:relative;
    margin: 0 2%;
    .niw_inner{
        width: 90%;
        height: 90%;
        ${(props: any) => props.node === 'f1' && props.status === 'empty' ?
        css`padding:0 5%;`
        :
        css`padding: 5%;`
    };
