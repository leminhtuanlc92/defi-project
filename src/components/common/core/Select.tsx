...
export default ({ listSelect, action, disabled = false, defaultSelect = `-- ${i18n.t('select')} --`, currentSelect }: SelectProps) => {
    const [show, setShow] = useState(false)
    return (
        <SelectInput disabled={disabled}>
            <div id="sli_current_select" onClick={() => setShow(!show)}>
                <span id="slics_name">{currentSelect && currentSelect.title !== '' ? currentSelect.title : defaultSelect}</span>
                ...
                <DropList>
                    <div id="slidl_wrap">
                        {listSelect.map((item, index) => {
                            return (
                                <DropListItem key={index} onClick={() => action(item)}>
